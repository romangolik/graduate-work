const getErrorMessageModalHtml = (data) => {
    return `<div class="modal-window error-message-modal">
              <button
                class="modal-window__close-button">
                <svg class="modal-window__close-button-icon"><use xlink:href="#delete-icon"></use></svg>
              </button>
              <div class="modal-window__content">
                <svg class="error-message-modal__icon" width="71" height="70" viewBox="0 0 71 70" fill="none" xmlns="http://www.w3.org/2000/svg">
                ...
                <h2 class="error-message-modal__heading">${data.heading}</h2>
                <p class="error-message-modal__paragraph">${data.text}</p>
              </div>
            </div>`
}
